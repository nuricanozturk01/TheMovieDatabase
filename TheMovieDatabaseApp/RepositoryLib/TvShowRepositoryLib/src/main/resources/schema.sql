...
select ts.tvshow_id,
       ...
       td.season_count,
       td.poster_path,
       group_concat(distinct tg.genre_name separator ', ')     as genres,
       group_concat(distinct tcom.company_name separator ', ') as companies,
       group_concat(distinct tc.country_name separator ', ')   as countries
from tv_show as ts
...
DELIMITER $$
create procedure insertGenre(in p_detailId int, in p_name varchar(45))
begin
insert into tv_show_genre(tvshow_detail_id, genre_name) values (p_detailId, p_name);
end $$
DELIMITER ;

DELIMITER $$
create procedure insertProductionCompany(in p_detailId int, in p_name varchar(45))
begin
insert into tv_show_production_company(tvshow_detail_id, company_name) values (p_detailId, p_name);
end $$
DELIMITER ;

DELIMITER $$
create procedure insertProductionCountry(in p_detailId int, in p_name varchar(45))
begin
insert into tv_show_production_country(tvshow_detail_id, country_name) values (p_detailId, p_name);
end $$
DELIMITER ;
...
DELIMITER $$
create procedure findByGenre(in p_genre varchar(45))
begin
select * from all_info where genres like concat('%', p_genre, ',%');
end;
DELIMITER ;

DELIMITER $$
create procedure findByCompany(in p_company varchar(45))
begin
select * from all_info where companies like concat('%', p_company, ',%');
end;
DELIMITER ;

DELIMITER $$
create procedure findByCountry(in p_country varchar(45))
begin
select * from all_info where countries like concat('%', p_country, ',%');
end;
DELIMITER ;
